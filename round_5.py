...
class Trader:
    def __init__(self):
        ...
        self.above_max = {b: False for b in self.bcontents}
        self.below_min = {b: False for b in self.bcontents}
        self.basket_window = {b: [] for b in self.bcontents}
        self.basket_window_size = 10
...
########################## BASKETS functions ############################
    ...
    #     return decision
    def baskets_logic(self, state: TradingState, fair_price, basket):
        synth_slope = 0
        for content in self.bcontents[basket].keys():
            synth_slope += self.bcontents[basket][content]*(self.history[content][-1]-self.history[content][-31])/30
        
        thresh = 1
        order_depth: OrderDepth = state.order_depths[basket]
        ...
        to_buy = self.max_positions[basket] - position
        to_sell = self.max_positions[basket] + position
        max_buy_price = fair_price - 2 if position > self.max_positions[basket] * 0.5 else fair_price
        min_sell_price = fair_price + 2 if position < self.max_positions[basket] * -0.5 else fair_price     
        
        if synth_slope > thresh and not self.above_max[basket]:
            self.above_max[basket] = True
        elif synth_slope < 0 and self.above_max[basket]:
            self.above_max[basket] = False
            # for price, volume in buy_orders:
            #     if to_sell > 0 and price >= min_sell_price:
            #         quantity = min(to_sell, volume)
            #         orders.append(Order(basket,int(price), -quantity))
            #         to_sell -= quantity
            #         logger.print(to_sell, quantity, price)

            #     if to_sell > 0:
            #         popular_sell_price = min(sell_orders, key=lambda tup: tup[1])[0]
            #         price = max(min_sell_price, popular_sell_price - 1)
            #         orders.append(Order(basket,int(price), -to_sell)) 
            price, volume = buy_orders[0]
            quantity = min(to_sell, volume)
            orders.append(Order(basket,int(price), -quantity))

        
        if synth_slope < -thresh and not self.below_min[basket]:
            self.below_min[basket] = True
        elif synth_slope > 0 and self.below_min[basket]:
            self.below_min[basket] = False 
            # for price, volume in sell_orders:
            #     if to_buy > 0 and price <= max_buy_price:
            #         quantity = min(to_buy, -volume)
            #         orders.append(Order(basket,int(price), quantity))
            #         to_buy -= quantity
            #         # logger.print(to_buy, quantity, price)

            #     if to_buy > 0:
            #         popular_buy_price = max(buy_orders, key=lambda tup: tup[1])[0]
            #         price = min(max_buy_price, popular_buy_price + 1)
            #         orders.append(Order(basket,int(price), to_buy))
            price, volume = sell_orders[0]
            quantity = min(to_buy, -volume)
            orders.append(Order(basket,int(price), -quantity))

        if(len(buy_orders)==0 or len(sell_orders)==0): return orders

        self.basket_window[basket].append(abs(position) == self.max_positions[basket])
        if len(self.kelp_window) > self.kelp_window_size:
            self.kelp_window.pop(0)

        soft_liquidate = len(self.basket_window[basket]) == self.basket_window_size and sum(self.basket_window[basket]) >= self.basket_window_size / 2 and self.basket_window[basket][-1]
        hard_liquidate = len(self.basket_window[basket]) == self.basket_window_size and all(self.basket_window[basket])

        if self.above_max[basket] or self.below_min[basket]:
            if to_buy > 0 and hard_liquidate:
                quantity = to_buy // 2
                orders.append(Order(basket,int(fair_price), quantity))
                to_buy -= quantity

            if to_buy > 0 and soft_liquidate:
                quantity = to_buy // 2
                orders.append(Order(basket,int(fair_price - 2), quantity))
                to_buy -= quantity
            ...
            if to_sell > 0:
                ...
                orders.append(Order(basket,int(price), -to_sell))        
        logger.print(orders)
        logger.print(self.above_max, self.below_min)
        return orders
...
########################## ROCK LOGIC ############################    
    def rock_logic(self, state, product):
        imbalance = self.imbalance[product]
        fair_price = self.fair_price[product]
        rsi = Trader.calculate_rsi(self, product, period=40)
        spread = self.spreads[product]
        ...
        # Calculate buy/sell prices and volumes
        buy_price = int(fair_price - spread*2*imbalance)
        sell_price = int(fair_price + spread*3*(1-imbalance))
        position = state.position.get(product, 0)                   
        buy_volume = int((max_position - position)*0.8)
        sell_volume = int((max_position + position)*0.8)
        # Simple momentum logic
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            # if product not in self.bcontents:
            #     continue
            order_depth: OrderDepth = state.order_depths[product]
            # Estimate fair value using best bid/ask
        ...
        for product in ['PICNIC_BASKET1']:
            orders: List[Order] = []
            if len(self.history[product])>30:
                orders = self.baskets_logic(state, self.fair_price[product], product)
                result[product] = orders
        ...
        for product in ['VOLCANIC_ROCK']:
            orders: List[Order] = []
            buy_price, sell_price, buy_volume, sell_volume = Trader.rock_logic(self, state, product)
            if buy_volume > 0:
                print(f"Placing BUY order: {buy_volume} @ {buy_price}")
                orders.append(Order(product, buy_price, buy_volume))
